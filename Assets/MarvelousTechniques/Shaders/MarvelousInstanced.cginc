...
/*#if USE_DIR_LIGHT
uniform half3 IP(InstProperties, _LightDirF); 
uniform half3 IP(InstProperties, _LightDirT);
uniform half3 IP(InstProperties, _LightDirR);
#endif*/
...
#ifdef USE_GRADIENT
	half gradient = saturate((ypos - IP(InstProperties, _GradientStartY)) / IP(InstProperties,_GradientHeight));
	f_color = lerp(IP(InstProperties, _FrontColorBottom),f_color,gradient);
	r_color = lerp(IP(InstProperties, _RightColorBottom),r_color,gradient);
	t_color = lerp(IP(InstProperties,_TopColorBottom),t_color,gradient);
	rimColor  = lerp(IP(InstProperties, _RimColorBottom),rimColor,gradient);
#endif
...
#if USE_DIR_LIGHT
	half f_d = acos((dot(IP(InstProperties, _LightDirF),normal)))/1.5708;
	half r_d = acos((dot(IP(InstProperties, _LightDirR),normal)))/1.5708;
	half t_d = acos((dot(IP(InstProperties, _LightDirT),normal)))/1.5708;
...
CL_OUT_WPOS customLightingWPosVertSimple(CL_IN v, half3 mainColor,half3 rimColor, half rimPower,half rightLight, half frontLight, half topLight, half3 ambientColor, float ambientPower){
	...
	half lpos = o.wpos.y;
		...
		lpos = v.local_height.y;
	#endif
	half gradient = saturate((lpos - IP(InstProperties, _GradientStartY)) / IP(InstProperties, _GradientHeight));
	o.lighting = lerp(IP(InstProperties,_MainColorBottom),mainColor,half3(gradient,gradient,gradient))*o.lighting+(ambientColor*ambientPower);
#else
...
fixed4 calculteDistanceLight(fixed4 currentLight,CL_OUT_WPOS v){
	half l =length(IP(InstProperties, _LightPos) - v.wpos);
 	l = clamp(min(l,IP(InstProperties, _LightMaxDistance))/IP(InstProperties, _LightMaxDistance),0,1);
