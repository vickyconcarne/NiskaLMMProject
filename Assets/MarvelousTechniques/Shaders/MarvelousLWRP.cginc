...
#if defined(USE_LIGHTMAP) || defined(USE_REALTIME_SHADOWS) || (defined(USE_SPECULAR) && defined(USE_SPECULAR_PIXEL_SHADING))
...
CL_OUT_WPOS calculateLighting(CL_IN v,half3 rimColor,half rimPower,half3 f_color,half3 r_color,half3 t_color){
	...
	half3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
  	half3 viewDirection = normalize(UnityWorldSpaceViewDir(wpos));
...
half shadowAttenuation(CL_OUT_WPOS v){
#ifdef USE_REALTIME_SHADOWS
...
#ifdef USE_DIST_LIGHT
#ifdef USE_DIST_LIGHT_MULTI
inline half4 calculateOneDistanceLight(CL_OUT_WPOS v, half4 lightPos){
...
half4 calculateDistanceLight(half4 currentLight, CL_OUT_WPOS v){
	half4 light=half4(0,0,0,0);
	for(int i=0; i<_LightPositionsCount; i++){
		light += calculateOneDistanceLight(v, _LightPositions[i]);
	}
	light/=_LightPositionsCount;

#ifdef DIST_LIGHT_ADDITIVE
 	return currentLight*0.5 + light;
 #else
...
half4 customLightingFrag(CL_OUT_WPOS v, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower){
	...
	half4 textureColor = half4(1,1,1,1);
        ...
        half3 bakedColorTex = SampleLightmap(lm_uv, v.normal)*shadowAttenuation(v);
	//half4 bakedColorTex  = SAMPLE_TEXTURE2D(unity_Lightmap, samplerunity_Lightmap, lm_uv)*shadowAttenuation(v);
        //half4 bakedColorTex  = SampleLightmap(lm_uv, v.normal);
		realLightmapPower = lerp(half3(1,1,1),clamp(/*DecodeLightmap(bakedColorTex)*/bakedColorTex+(_ShadowPower),0,1),lightmapPower);
...
#ifdef USE_DIST_LIGHT
  	outColor = calculateDistanceLight(outColor, v);
#endif

#if defined(USE_SPECULAR) && defined(USE_SPECULAR_PIXEL_SHADING)
	half3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
  	half3 viewDirection = normalize(UnityWorldSpaceViewDir(v.wpos));
...
#ifdef USE_FOG
half4 customLightingHardFogFrag(CL_OUT_WPOS v,float fogYStartPos, half3 fogColor, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower){

	half4 outColor = customLightingFrag(v, lightTint, useLightmap, lightmapPower, lightmapColor, _ShadowPower);
	return  half4(lerp( fogColor, outColor.xyz, v.wpos.y > fogYStartPos),0);
}

half4 customLightingSoftFogFrag(CL_OUT_WPOS v, half3 fogColor, half fogHeight, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower){
	half4 outColor = customLightingFrag(v, lightTint, useLightmap, lightmapPower, lightmapColor, _ShadowPower);
