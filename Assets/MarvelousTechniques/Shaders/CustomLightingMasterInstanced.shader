...
#if USE_DIR_LIGHT
                UNITY_DEFINE_INSTANCED_PROP (half3, _LightDirF)
                UNITY_DEFINE_INSTANCED_PROP (half3, _LightDirT)  
                UNITY_DEFINE_INSTANCED_PROP (half3, _LightDirR)
#endif
...
UNITY_INSTANCING_BUFFER_END(InstProperties)
				...
				CL_OUT_WPOS vert(CL_IN v) {
					UNITY_SETUP_INSTANCE_ID(v);
								...
								float cameraVertDist = length(_WorldSpaceCameraPos - o.wpos)*IP(InstProperties,_FogDensity); 
								#else
								float cameraVertDist = length(_FogStaticStartPos - o.wpos)*IP(InstProperties,_FogDensity); 
								#endif
								o.color.w = saturate((IP(InstProperties,_FogEnd) - cameraVertDist) / (IP(InstProperties,_FogEnd) - IP(InstProperties,_FogStart)));	
								UNITY_TRANSFER_INSTANCE_ID(v, o);
                                return o;		
							#endif // USE_DIST_FOG
						#else // USE_FOG
							CL_OUT_WPOS o = customLightingWPosVertSimple(v, 
							IP(InstProperties,_MainColor), 
							IP(InstProperties,_RimColor), 
							IP(InstProperties,_RimPower), 
							IP(InstProperties,_RightLight), 
							...
							#endif
						#else
							CL_OUT_WPOS o= customLightingWPosVert(v, 
							IP(InstProperties,_RimColor), 
							IP(InstProperties,_RimPower), 
							IP(InstProperties,_RightColor), 
							IP(InstProperties,_FrontColor), 
				...
				fixed4 frag(CL_OUT_WPOS v) : COLOR {
                    ...
                    UNITY_SETUP_INSTANCE_ID(v);
						...
						#ifdef CUTOUT
							clip (o.a - IP(InstProperties, _Cutoff));
						#endif
